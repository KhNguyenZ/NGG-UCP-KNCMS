...
<div class="container-fluid">
    <div class="card shadow mb-4">
        <div class="card-header py-3">
            <h6 class="m-0 font-weight-bold text-primary">Xếp hạng Vehicle Voucher</h6>
        </div>
        <div class="card-body">
                ...
                <table id="basic-datatables" class="display table table-striped table-hover">
                    <thead>
                        <tr>
                            <th>ID</th>
                            <th>Người Chơi</th>
                            <th>Vehicle Voucher</th>
                        </tr>
                    </thead>
                    <tbody>
                        <?php
                        foreach ($KNCMS->get_list("SELECT * FROM `accounts` WHERE `VehVoucher` > 0 ORDER BY VehVoucher DESC LIMIT 100") as $row) { ?>
                            <tr>
                                <td>
                                    <?= $row['id'] ?>
                                </td>
                                ...
                                <td>
                                    <span class="badge badge-pill badge-info"><?= $row['VehVoucher'] ?></span>
                                </td>
                            </tr>
                        <?php } ?>
                    </tbody>
