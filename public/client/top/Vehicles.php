...
<div class="container-fluid">
    <div class="card shadow mb-4">
        <div class="card-header py-3">
            <h6 class="m-0 font-weight-bold text-primary">Xếp hạng Vehicles</h6>
        </div>
        <div class="card-body">
            <div class="table-responsive">
                <table id="basic-datatables" class="display table table-striped table-hover">
                    ...
                    <tbody>
                        <?php
                        foreach ($KNCMS->get_list("SELECT * FROM `accounts` WHERE `Registered` > 0") as $row) {
                            $uidz = $row['id'];
                            $total = mysqli_fetch_assoc($KNCMS->query("SELECT COUNT(*) FROM `vehicles` WHERE `sqlID` = '$uidz'"))['COUNT(*)'];
                            if ($total > 0) {
                        ?>
                                <tr>
                                    <td>
