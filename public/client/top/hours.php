...
<div class="container-fluid">
    <div class="card shadow mb-4">
        <div class="card-header py-3">
            <h6 class="m-0 font-weight-bold text-primary">Xếp hạng giờ chơi</h6>
        </div>
        <div class="card-body">
            <div class="table-responsive">
                <table id="basic-datatables" class="display table table-striped table-hover">
                    ...
                    <tbody>
                        <?php
                        foreach ($KNCMS->get_list("SELECT * FROM `accounts` WHERE `ConnectedTime` > 0 ORDER BY ConnectedTime DESC LIMIT 100") as $row) { ?>
                            <tr>
                                <td>
                                    <?= $row['id'] ?>
                                </td>
                                ...
                                <td>
                                    <span class="badge badge-pill badge-info"><?= $row['ConnectedTime'] ?></span>
                                </td>
                            </tr>
                        <?php } ?>
                    </tbody>
